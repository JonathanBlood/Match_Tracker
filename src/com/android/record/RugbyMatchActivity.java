...
/**
 * This class is responsible for the recording of Rugby matches.
 * 
 * @author Jonathan
 * 
 */
public class RugbyMatchActivity extends Activity
{

	private static final String SPORT = "Rugby";
	...
	private TextView matchInfoTV, team1NameTV, team2NameTV, team1ScoreTV, team2ScoreTV;
	private Chronometer matchChron; // Match timer.
	private Spinner matchEventSpinner; // Display the different match events.
	...
	private int minutesPerHalf, tryValue, conversionValue, penaltyValue, dropGoalValue, team1OverallScore = 0,
			team2OverallScore = 0;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		setContentView(R.layout.rugby_match);
		...
		tryValue = getParameters.getIntExtra("TryValue", 5);
		conversionValue = getParameters.getIntExtra("ConversionValue", 2);
		penaltyValue = getParameters.getIntExtra("PenaltyValue", 3);
		dropGoalValue = getParameters.getIntExtra("DropGoalValue", 3);
		...
		team1NameTV = (TextView) this.findViewById(R.id.team1NameTVRM);
		team1NameTV.setText(team1);
		team1ScoreTV = (TextView) this.findViewById(R.id.team1GoalTVRM);
		team1ScoreTV.setText(Integer.toString(team1OverallScore));
		team2NameTV = (TextView) this.findViewById(R.id.team2NameTVRM);
		team2NameTV.setText(team2);
		team2ScoreTV = (TextView) this.findViewById(R.id.team2GoalTVRM);
		...
		onePlayerEventDG = new Dialog(RugbyMatchActivity.this);
		...
		twoPlayerEventDG = new Dialog(RugbyMatchActivity.this);
		...
		noteDG = new Dialog(RugbyMatchActivity.this);
		noteDG.setContentView(R.layout.note_dialog);
		noteDG.setTitle("Add note: ");
		noteDG.setCancelable(true);
		this.noteDialogSetup();
				
		addNoteBn = (ImageButton) findViewById(R.id.addNoteRM);
		this.addNoteButton();

		matchEvents = new ArrayList<MatchEvent>();

		// Timer elements
		startBn = (ImageButton) this.findViewById(R.id.startButtonRM);
		this.startButton();
		stopBn = (ImageButton) this.findViewById(R.id.stopButtonRM);
		stopBn.setEnabled(false);
		this.stopButton();
		matchChron = (Chronometer) findViewById(R.id.chronometerRM);
		this.onChronTick();

		// Match events elements.
		matchEventSpinner = (Spinner) findViewById(R.id.matchEventSpinnerRM);
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.RugbyMatchEvent_array,
		...
		matchInfoTV = (TextView) this.findViewById(R.id.matchInfoTVRM);
		undoEventBn = (ImageButton) this.findViewById(R.id.undoEventButtonRM);
		this.undoEventButton();
		finishMatchBn = (ImageButton) this.findViewById(R.id.finishMatchRM);
	...
	private void onePlayerEventDialogSetup()
	{
		...
		createMatchEventBN.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				// If try, conversion, penalty, drop goal event update GUI to
				// display new score.
				if ("Try".equals(eventType))
					updateTryScore(eventTeamSel);
				else if ("Conversion".equals(eventType))
					updateConversionScore(eventTeamSel);
				else if ("Penalty".equals(eventType))
					updatePenaltyScore(eventTeamSel);
				else if ("Drop Goal".equals(eventType))
					updateDropGoalScore(eventTeamSel);
		...
		undoEventBn.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				if (matchEvents.size() >= 1)
				{
					...
					// Check if the event is a try, conversion, penalty, drop
					// goal
					// so we can then undo the score.
					if ("Try".equals(aMatchEvent.getType()))
						undoTryScore(aMatchEvent.getTeam());
					else if ("Conversion".equals(aMatchEvent.getType()))
						undoConversionScore(aMatchEvent.getTeam());
					else if ("Penalty".equals(aMatchEvent.getType()))
						undoPenaltyScore(aMatchEvent.getTeam());
					else if ("Drop Goal".equals(aMatchEvent.getType()))
						undoDropGoalScore(aMatchEvent.getTeam());
	...
	private void finishMatchButton()
	{
		finishMatchBn.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v)
			{
				...
				Intent i = new Intent(RugbyMatchActivity.this, MainMenuActivity.class);
	...
	/**
	 * If try is called it updates the score of the match.
	 * 
	 * @param aTeam
	 */
	private void updateTryScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore += tryValue;
			team1ScoreTV.setText(Integer.toString(team1OverallScore));
		}
		else if (team2.equals(aTeam))
		{
			team2OverallScore += tryValue;
			team2ScoreTV.setText(Integer.toString(team2OverallScore));
		}
	}

	/**
	 * If conversion is called it updates the score of the match.
	 * 
	 * @param aTeam
	 */
	private void updateConversionScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore += conversionValue;
			team1ScoreTV.setText(Integer.toString(team1OverallScore));
		}
		else if (team2.equals(aTeam))
		{
			team2OverallScore += conversionValue;
			team2ScoreTV.setText(Integer.toString(team2OverallScore));
		}
	}

	/**
	 * If penalty is called it updates the score of the match.
	 * 
	 * @param aTeam
	 */
	private void updatePenaltyScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore += penaltyValue;
			team1ScoreTV.setText(Integer.toString(team1OverallScore));
		}
		else if (team2.equals(aTeam))
		{
			team2OverallScore += penaltyValue;
			team2ScoreTV.setText(Integer.toString(team2OverallScore));
		}
	}

	/**
	 * If drop goal is called it updates the score of the match.
	 * 
	 * @param aTeam
	 */
	private void updateDropGoalScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore += dropGoalValue;
			team1ScoreTV.setText(Integer.toString(team1OverallScore));
		}
		else if (team2.equals(aTeam))
		{
			team2OverallScore += dropGoalValue;
			team2ScoreTV.setText(Integer.toString(team2OverallScore));
		}
	}

	/**
	 * If undo button is called and type == try then undo the score for the team.
	 * 
	 * @param aTeam
	 */
	private void undoTryScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore -= tryValue;
	...
	/**
	 * If undo button is called and type == conversion then undo the score for the team.
	 * 
	 * @param aTeam
	 */
	private void undoConversionScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore -= conversionValue;
	...
	/**
	 * If undo button is called and type == penalty then undo the score for the team.
	 * 
	 * @param aTeam
	 */
	private void undoPenaltyScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore -= penaltyValue;
	...
	/**
	 * If undo button is called and type == drop goal then undo the score for the team.
	 * 
	 * @param aTeam
	 */
	private void undoDropGoalScore(String aTeam)
	{
		if (team1.equals(aTeam))
		{
			team1OverallScore -= dropGoalValue;
	...
	{
		MediaPlayer mp = MediaPlayer.create(RugbyMatchActivity.this, R.raw.buzzer);
		mp.start();
	...
	private class ActionBarClickListener implements OnClickListener
	{
		...
		public void onClick(View v)
		{
			...
			case R.id.home_button:
				i = new Intent(RugbyMatchActivity.this, MainMenuActivity.class);
				i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
				break;
			case R.id.login_button:
				...
				{
					i = new Intent(RugbyMatchActivity.this, LoginActivity.class);
				}
				break;
			case R.id.sync_button:
				...
				if (!serviceSsettings.getBoolean("serviceStarted", false))
				{
					...
					Intent syncIntent = new Intent(RugbyMatchActivity.this, SyncService.class);
	...
	private class SpinnerItemSelectedListener implements OnItemSelectedListener
	{
		...
		public void onItemSelected(AdapterView<?> parent, View view, int pos, long id)
		{
			...
			else if (item.equals("Try"))
			{
				eventType = "Try";
				onePlayerEventDG.show();
			}
			else if (item.equals("Conversion"))
			{
				eventType = "Conversion";
				onePlayerEventDG.show();
			}
			else if (item.equals("Penalty"))
			{
				eventType = "Penalty";
				onePlayerEventDG.show();
			}
			else if (item.equals("Drop Goal"))
			{
				eventType = "Drop Goal";
				onePlayerEventDG.show();
			}
