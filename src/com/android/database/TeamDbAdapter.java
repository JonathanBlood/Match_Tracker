...
public class TeamDbAdapter
{
	...
	/**
	 * Delete a team from the Team table.
	 * 
	 * @param teamName
	 */
	public void deleteTeam(String teamName)
	{
		try
		{
			db.delete(DB_TABLE, KEY_NAME + "=" + teamName, null);
		}
		catch (Exception e)
		{
			Log.e("DB ERROR", e.toString());
	...
	/**
	 * Updata a team in the Team table.
	 * 
	 * @param name
	 * @param sport
	 * @param wins
	 ...
	 * @param total_points
	 */
	public void updateTeam(String name, String sport, int wins, int losses, int draws, int total_points)
	{
		...
		try
		{
			db.update(DB_TABLE, values, KEY_NAME + "=" + "'" + name + "'", null);
		}
		catch (Exception e)
		{
			Log.e("DB ERROR", e.toString());
	...
	/**
	 * Update a single column in the Team table with a type String.
	 * 
	 * @param teamName
	 * @param column
	 * @param updatedValue
	 *            - of type String.
	 */
	public void updateSingleColumn(String teamName, String column, String updatedValue)
	{
	...
	/**
	 * Get the column values for a specific column provided of type Integer.
	 * 
	 * @param column
	 * @param duplicates
	 * @return ArrayList<Integer> containing results of query.
	 */
	public ArrayList<Integer> getColumnValuesInt(String column, boolean duplicates)
	{
		Cursor cursor = null;
		...
		if (cursor.moveToFirst())
		{
			...
			do
			{
				columnList.add(cursor.getInt(columnIndex));
			}
	...
	public int getColumnValueForTeamInt(String column, String teamName)
	{
		...
		try
		{
			cursor = db.rawQuery("SELECT " + column + " FROM " + DB_TABLE + " WHERE " + KEY_NAME + "='" + teamName
					+ "'", null);
		}
		catch (Exception e)
		{
	...
	public ArrayList<TeamInfo> getAllValuesForGivenSport(String orderCol, String sport)
	{
		...
		try
		{
			cursor = db.rawQuery("SELECT * FROM " + DB_TABLE + " WHERE sport='" + sport + "' ORDER BY " + orderCol
					+ " DESC", null);
		}
		catch (Exception e)
		{
		...
		if (cursor.moveToFirst())
		{
			...
			do
			{
				...
				TeamInfo teamInfo = new TeamInfo(cursor.getString(nameIndex), cursor.getInt(winsIndex),
						cursor.getInt(lossesIndex), cursor.getInt(drawsIndex), cursor.getInt(totalPointsIndex));
				result.add(teamInfo);
			}
			// move the cursor's pointer up one position.
			while (cursor.moveToNext());
